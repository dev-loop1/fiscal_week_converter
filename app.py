...
def process_data_with_polars(source_file, date_col, value_col, date_format):
    ...
    lf_split_calculated = lf_to_split.with_columns(
        pl.col("date_obj").dt.month_end().alias("last_day_of_month"),
    ).with_columns(
        ((pl.col("last_day_of_month") - pl.col("date_obj")).dt.total_days().cast(pl.Int8) + 1).alias("days_in_first_week"),
    ).with_columns(
        first_week_value=(pl.col(value_col) / 7 * pl.col("days_in_first_week")).round(2),
    ).with_columns(
        second_week_value=(pl.col(value_col) - pl.col("first_week_value"))
    )
    lf_split1 = lf_split_calculated.with_columns(pl.col("first_week_value").alias(value_col))
    lf_split2 = lf_split_calculated.with_columns(
        ...
        if file_extension == 'csv':
            with st.spinner("Processing file... Please wait."):
                try:
                    ...
                    v_col1.metric("Sum of Original Values", f"{original_sum:,.2f}")
                    v_col2.metric("Sum of Processed Values", f"{processed_sum:,.2f}")
                    if np.isclose(original_sum, processed_sum):
                        st.success("Verification Successful!")
                    else:
                        st.warning("Verification Failed: Sums do not match.")
                    st.subheader("Processed Data Preview")
                    st.dataframe(result_df_pl.to_pandas().head())
